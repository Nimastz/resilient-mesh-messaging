...
# 🧪 Routing Service – Config Quick Test

These tests verify that the router **actually obeys the YAML config** and that the IDS behavior is  **tunable and safe** .

### 1️⃣ Run the tests

From project root:

<pre class="overflow-visible!" data-start="320" data-end="394"><div class="contain-inline-size rounded-2xl corner-superellipse/1.1 relative bg-token-sidebar-surface-primary"><div class="sticky top-9"><div class="absolute end-0 bottom-0 flex h-9 items-center pe-2"><div class="bg-token-bg-elevated-secondary text-token-text-secondary flex items-center gap-4 rounded-sm px-2 font-sans text-xs"></div></div></div><div class="overflow-y-auto p-4" dir="ltr"><code class="whitespace-pre! language-bash"><span><span>pytest services/routing_service/test/routing_config_test.py -v
</span></span></code></div></div></pre>

No external services are required; the tests use the routing app in-process and monkeypatch different configs.
...
**Forwarding vs. final delivery**

* `test_on_chunk_received_action_final_by_default`

  With `forwarding_enabled: false`, incoming chunks return `{accepted: true, action: "final"}`.
* `test_on_chunk_received_action_forward_when_enabled`
  ...
  This proves routing behavior is fully controlled by config, not hard-coded.

**IDS tuning (blocking + duplicates)**

* `test_block_peer_after_threshold`

  With `block_peer_after` set low, repeated suspicious events for a peer cause it to be treated as blocked by the rate limiter.

  This validates the “escalation” path from logging → temporary peer blocking.
* `test_duplicate_ttl_eviction`

  With a small `duplicate_suppression_ttl`, a `msg_id` is treated as duplicate within the window, but **forgotten** after the TTL expires.

  This proves duplicate suppression is time-bounded and won’t grow unbounded in memory.

**Queue saturation / DoS protection**

* `test_queue_full_returns_db_error`

  With `max_queue_size` forced to 0, the very first enqueue fails with `500 DB_ERROR`.

  This verifies that the router **detects a full queue** and fails closed instead of silently dropping or corrupting state.
